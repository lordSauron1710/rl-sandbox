...
- For public self-hosted deployments, set `RLV_ACCESS_TOKEN`.
- Leaving `RLV_ACCESS_TOKEN` unset in production is only acceptable when
  `RLV_DEPLOYMENT_BOUNDARY=private` and the backend stays behind a trusted
